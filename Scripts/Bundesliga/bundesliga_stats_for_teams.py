...
def aggregate_team_stats(team_fixture_data):
    ...
    for col in df.columns:
        if col != EVIDENCE_KEY and col not in ["fixture_id", "fixture", "fixture_date_utc", "fixture_date",
...
def save_team_outputs(per_fixture_df, aggregate_df):
    json_path1 = OUTPUT_DIR / f"Bundesliga_team_fixture_stats_{SEASON}.json"
    csv_path1 = OUTPUT_DIR / f"Bundesliga_team_fixture_stats_{SEASON}.csv"
    per_fixture_df.to_json(json_path1, orient="records", indent=4)
    per_fixture_df.to_csv(csv_path1, index=False)

    json_path2 = OUTPUT_DIR / f"Bundesliga_team_aggregate_stats_{SEASON}.json"
    csv_path2 = OUTPUT_DIR / f"Bundesliga_team_aggregate_stats_{SEASON}.csv"
