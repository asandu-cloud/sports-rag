...
from lineup_context import get_confirmed_lineup_context_for_event  # noqa: E402
...
async def _generate_match_reads_for_hub(league: str, target_date: date):
    ...
    Confirmed-lineup amendments are deliberately separate: they need an API
    response containing both verified XIs and are generated by
    ``_generate_confirmed_lineup_match_reads_for_hub`` below.
    """
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(
        _rag_executor,
...
async def _generate_confirmed_lineup_match_reads_for_hub(
    league: str,
    target_date: date,
    *,
    fixture_ids: set[str] | None = None,
):
    """Persist only genuinely verified-XI Match Read amendments.

    The dispatcher skips fixtures whose API response does not contain two
    matching 11-player starting XIs.  It therefore cannot turn a merely
    expected lineup or a partial provider payload into a public amendment.
    """
    loop = asyncio.get_running_loop()
    def _is_candidate(event: dict) -> bool:
        ...
        return str(fixture_id or "").strip() in fixture_ids
...
def _is_lineup_match_read_window(event: dict, now: datetime) -> bool:
    """Whether a fixture is close enough for a verified-XI amendment.

    API-Football normally exposes starting XIs about an hour before kickoff.
    We start looking slightly earlier and stop before kickoff so an API result
    cannot create a newly labelled pre-game update for a match in progress.
    """
    kickoff = event.get("commence_time") if isinstance(event, dict) else None
    if not kickoff:
        return False
    try:
    ...
    minutes_to_kickoff = (kickoff_at - now).total_seconds() / 60
    return 10 <= minutes_to_kickoff <= 100
...
class AutoPush(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        # Prevent a scheduler tick from re-rendering a hub when API-Football
        # returns the exact same confirmed-XI Match Read again.  A real late
        # lineup correction creates a new immutable read id and is processed.
        self._seen_confirmed_match_read_ids: set[int] = set()
    ...
    async def cog_load(self):
        ...
        if _match_read_hub_enabled():
            self.match_read_lineup_refresh.start()
    ...
    async def cog_unload(self):
        for task in [self.daily_parlay, self.value_scan, self.market_channels,
                     self.player_props_pre_match, self.match_read_lineup_refresh]:
            if task.is_running():
                task.cancel()
    ...
    # ===================================================================
    # MATCH READ LINEUP AMENDMENTS — verified XIs shortly before kickoff
    # ===================================================================

    @tasks.loop(minutes=10)
    async def match_read_lineup_refresh(self):
        """Amend public league hubs only after both official XIs are present.

        This task is dormant unless ``MATCH_READ_MODE=hub`` is explicitly
        configured.  It uses a narrow 10–100 minute pre-kickoff window,
        persists a confirmed-lineups version only for eligible fixtures, then
        refreshes the same Discord hub message.  The existing delivery bridge
        records the amended visible selections without overwriting prior
        public exposures.
        """
        if not _match_read_hub_enabled():
            return
        ...
        for league in active_leagues:
            try:
                events, _ = await loop.run_in_executor(
                    _rag_executor,
                    lambda current_league=league: fetch_events(
                        current_league,
                        target_date=target_date,
                    ),
                )
                ...
                candidate_ids = {
                    str(event.get("_fixture_id") or event.get("fixture_id") or event.get("id") or "").strip()
                    for event in day_events
                    if _is_lineup_match_read_window(event, now)
                }
                candidate_ids.discard("")
            except Exception:
                log.exception("Could not inspect lineup-update candidates for %s.", league)
                continue
            if not candidate_ids:
                continue

            try:
                confirmed_run = await _generate_confirmed_lineup_match_reads_for_hub(
                    league,
                    target_date,
                    fixture_ids=candidate_ids,
                )
            except Exception:
                log.exception("Confirmed-lineup Match Read generation failed for %s.", league)
                continue

            confirmed_ids = {
                int(record["id"])
                for record in confirmed_run.records
                if isinstance(record, dict) and str(record.get("id") or "").isdigit()
            }
            new_ids = confirmed_ids - self._seen_confirmed_match_read_ids
            self._seen_confirmed_match_read_ids.update(confirmed_ids)
            if not new_ids:
                continue

            # Re-price the remaining slate as well.  Otherwise the older
            # pre-match cards can exceed the shared freshness window by the
            # time the verified-XI amendment arrives.
            try:
                await _generate_match_reads_for_hub(league, target_date)
                delivered = await self._refresh_persisted_match_read_hubs([league], target_date)
            except Exception:
                log.exception("Could not refresh the Match Read hub after lineups for %s.", league)
                continue
            log.info(
                "Processed %d new confirmed-lineup Match Read(s) for %s; refreshed %d hub(s).",
                len(new_ids),
                league,
                delivered,
            )

    @match_read_lineup_refresh.before_loop
    async def _wait_match_read_lineups(self):
        await self.bot.wait_until_ready()
