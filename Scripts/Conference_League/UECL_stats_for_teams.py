...
LEAGUE_ID = 848  # Conference League
...
OUTPUT_DIR = Path('/Users/sanduandrei/Desktop/Betting_RAG/Output/UECL_teams')
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


# --- Fetch all fixture IDs and metadata (including final score + date) ---
def get_fixture_info():
    ...
    for f in data.get("response", []):
        ...
        # 🗓 match datetime in UTC from API
        # API-Football returns "fixture.date" like "2025-10-20T19:00:00+00:00"
        raw_dt = f["fixture"]["date"]  # ISO8601
        # keep both raw and a nice YYYY-MM-DD localizable token
        # we will store UTC iso and a short date for readability
        fixture_dt_utc_iso = raw_dt  # keep as-is
        fixture_dt_short = raw_dt[:10]  # "2025-10-20"

        # 🏁 final score in canonical "HomeTeam X - AwayTeam Y" form
        ...
        fixture_map[fid] = {
            ...
            "final_score": f"{home_goals}-{away_goals}",  # numeric only, legacy
            "final_score_string": final_score_string,      # readable, canonical
            "fixture_date_utc": fixture_dt_utc_iso,
            "fixture_date": fixture_dt_short,
        }
...
def fetch_fixture_team_stats(fixture_id, fixture_info):
    ...
    for entry in data.get("response", []):
        ...
        team_stats = {
            ...
            "fixture": fixture_info.get("fixture_name"),  # "Chelsea vs West Ham"
            "fixture_date_utc": fixture_info.get("fixture_date_utc"),  # full ISO
            "fixture_date": fixture_info.get("fixture_date"),          # "YYYY-MM-DD"
            ...
            "final_score": fixture_info.get("final_score"),  # "5-1"
...
# --- Aggregate per-team averages (season-level team profile features) ---
def aggregate_team_stats(team_fixture_data):
    df = pd.DataFrame(team_fixture_data)

    # Make sure these stay as strings (identifiers / metadata)
    string_cols = [
        "fixture_id",
        "fixture",
        "fixture_date_utc",
    ...
    team_for = (
        df.groupby("team", dropna=False)
          ...
          .reset_index()
          .rename(columns=lambda c: f"{c}_for" if c != "team" else c)
    )

    # --- 2) Team 'against' averages (what opponents did vs them)
    # pair each fixture with its opponent row
    paired = (
    ...
    team_against = (
        opp_stats.groupby("team", dropna=False)
                 .mean(numeric_only=True)
                 .reset_index()
                 .rename(columns=lambda c: f"{c}_against" if c != "team" else c)
    )
...
def save_team_outputs(per_fixture_df, aggregate_df):
    # Per-fixture stats for each team in each game this season
    ...
    # Aggregated stats per team across season
    ...
    print(f"💾 Saved {len(per_fixture_df)} per-fixture team rows and {len(aggregate_df)} team aggregates")
