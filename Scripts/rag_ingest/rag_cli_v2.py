...
def projected_total_cards_detail(
    ...
    """Rich cards projection — mirrors core.projections.projected_total_cards_detail().

    Returns dict with: total_cards, total_yellows, total_reds, season_total,
    recent_total, referee_modifier, lineup_card_context, uncertainty,
    confidence_bucket, warnings.
    """
    pw = SCORING_WEIGHTS["projection"]
    rw = SCORING_WEIGHTS["referee"]
    ...
    # Yellow/red split from profiles
    ...
    season_yellows = (h_yellows_pm + a_yellows_pm) if (h_yellows_pm is not None and a_yellows_pm is not None) else None
    season_reds = (h_reds_pm + a_reds_pm) if (h_reds_pm is not None and a_reds_pm is not None) else None

    # Recent yellow/red averages
    recent_yellows = None
    recent_reds = None
    h_ry, a_ry = hr.get("yellows_avg"), ar.get("yellows_avg")
    h_rr, a_rr = hr.get("reds_avg"), ar.get("reds_avg")
    if h_ry is not None and a_ry is not None:
        recent_yellows = h_ry + a_ry
    if h_rr is not None and a_rr is not None:
        recent_reds = h_rr + a_rr
    ...
    if lineup_ctx is not None:
        ...
        if h_risk and a_risk and (getattr(h_risk, "n_players", 0) > 0 or getattr(a_risk, "n_players", 0) > 0):
            combined_risk_90 = (
    ...
    # Yellow/red split from final total
