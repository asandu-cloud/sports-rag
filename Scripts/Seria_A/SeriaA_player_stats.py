...
def save_clustered_performances(clustered_players):
    ...
    csv_path = OUTPUT_DIR / f"SeriaA_player_clustered_performances_{SEASON}.csv"
...
def save_outputs(per_fixture_df, total_df):
    ...
    # Per fixture
    json_path1 = OUTPUT_DIR / f"SeriaA_player_fixture_stats_{SEASON}.json"
    csv_path1 = OUTPUT_DIR / f"SeriaA_player_fixture_stats_{SEASON}.csv"
    per_fixture_df.to_json(json_path1, orient="records", indent=4)
    per_fixture_df.to_csv(csv_path1, index=False)

    # Aggregated totals
    json_path2 = OUTPUT_DIR / f"Seria_A_player_total_stats_{SEASON}.json"
    csv_path2 = OUTPUT_DIR / f"Seria_A_player_total_stats_{SEASON}.csv"
